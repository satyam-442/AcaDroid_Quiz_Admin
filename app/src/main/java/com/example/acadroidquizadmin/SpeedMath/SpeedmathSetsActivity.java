...
public class SpeedmathSetsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new GridSpeedmathAdapter(sets, getIntent().getStringExtra("title"), new GridSpeedmathAdapter.GridListener() {
            @Override
            public void addSet() {
                loadingBar.show();
                FirebaseDatabase database = FirebaseDatabase.getInstance();
                final String uid = UUID.randomUUID().toString();
                database.getReference().child("SpeedMath").child(getIntent().getStringExtra("key")).child("Sets").child(uid).setValue("SET ID").addOnCompleteListener(new OnCompleteListener<Void>() {
                    @Override
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            ...
                            Toast.makeText(SpeedmathSetsActivity.this, "Set Added", Toast.LENGTH_SHORT).show();
                            loadingBar.dismiss();
                        } else {
                            Toast.makeText(SpeedmathSetsActivity.this, "Error occurred", Toast.LENGTH_SHORT).show();
                            loadingBar.dismiss();
                        }
                        loadingBar.dismiss();
                    }
            ...
            public void onLongClick(final int setNo, final String setId) {
                new AlertDialog.Builder(SpeedmathSetsActivity.this, R.style.Theme_AppCompat_Light_Dialog)
                                                    ...
                                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        ...
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            if (task.isSuccessful()) {
                                                                ...
                                                                Toast.makeText(SpeedmathSetsActivity.this, "Something went wrong", Toast.LENGTH_SHORT).show();
                                                            }
                                                            loadingBar.dismiss();
                                                        }
                                                    });
